...
const USER = { log: async (s) => {
  ...
  if (!clean.length) return;
  await window.ensureThreadOnFirstUser(clean[0]);
  const newMsgs = clean.map((t) => ({ id: gid(), role: "user", content: [{ type: "text", text: t }] }));
  window.state.messages.push(...newMsgs);
  const frag = document.createDocumentFragment();
  const newEls = newMsgs.map((m) => {
    const $row = window._createMessageRow(m), bubble = $row.find(".msg-bubble")[0];
    bubble.dataset.mid = m.id;
    return { rowEl: $row[0], bubbleEl: bubble, message: m };
  });
  newEls.forEach((item) => frag.appendChild(item.rowEl));
  window.el.messages.appendChild(frag);
  queueMicrotask(() => {
    newEls.forEach((item) => {
      window.renderMarkdown(item.bubbleEl, window.partsToText(item.message.content));
    });
    window.el.chat.scrollTo({ top: window.el.chat.scrollHeight, behavior: "smooth" });
    window.icons();
  });
  await THREAD.persist();
}, get PAT() {
...
const buildBody = () => {
  const msgs = [];
  if (window.USER.masterPrompt && !window.SUNE.ignore_master_prompt) msgs.push({ role: "system", content: [{ type: "text", text: window.USER.masterPrompt }] });
  if (window.SUNE.system_prompt) msgs.push({ role: "system", content: [{ type: "text", text: window.SUNE.system_prompt }] });
  msgs.push(...window.state.messages.filter((m) => m.role !== "system").map((m) => ({ role: m.role, content: m.content })));
  const b = payloadWithSampling({ model: window.SUNE.model.replace(/^(or:|oai:|g:|cla:|cf:)/, ""), messages: msgs, stream: true });
  if (window.SUNE.json_output) {
    let s;
    try {
      s = JSON.parse(window.SUNE.json_schema || "null");
    } catch {
      s = null;
    }
  ...
  if (window.SUNE.img_output) b.modalities = ["image", "text"];
  b.reasoning = { ...window.SUNE.reasoning_effort && window.SUNE.reasoning_effort !== "default" ? { effort: window.SUNE.reasoning_effort } : {}, exclude: !window.SUNE.include_thoughts };
  if (window.SUNE.verbosity) b.verbosity = window.SUNE.verbosity;
  return b;
};
async function askOpenRouterStreaming(onDelta, streamId) {
  const model = window.SUNE.model, provider = model.startsWith("oai:") ? "openai" : model.startsWith("g:") ? "google" : model.startsWith("cla:") ? "claude" : model.startsWith("cf:") ? "cloudflare" : model.startsWith("or:") ? "openrouter" : window.USER.provider, apiKey = provider === "openai" ? window.USER.apiKeyOpenAI : provider === "google" ? window.USER.apiKeyGoogle : provider === "claude" ? window.USER.apiKeyClaude : provider === "cloudflare" ? window.USER.apiKeyCloudflare : window.USER.apiKeyOpenRouter;
  if (!apiKey) {
    onDelta({ text: localDemoReply() }, true);
    return { ok: true, rid: streamId || null };
  }
  const r = { rid: streamId || sid(), seq: -1, done: false, signaled: false, ws: null };
...
async function generateTitleWithAI(messages) {
  const model = window.USER.titleModel, apiKey = window.USER.apiKeyOpenRouter;
...
async function syncActiveThread() {
  const id = window.THREAD.getLastAssistantMessageId();
  if (!id) return false;
  if (await cacheStore.getItem(id) === "done") {
    if (window.state.busy) {
      window.setBtnSend();
      window.state.busy = false;
      window.state.controller = null;
    }
    return false;
  }
  if (!window.state.busy) {
  ...
  const finalise = (t, c) => {
    window.renderMarkdown(bubble, t, { enhance: false });
    window.enhanceCodeBlocks(bubble, true);
    const i = window.state.messages.findIndex((x) => x.id === id);
    if (i >= 0) window.state.messages[i].content = c;
    else window.state.messages.push({ id, role: "assistant", content: c, ...window.activeMeta() });
    window.THREAD.persist();
    window.setBtnSend();
    window.state.busy = false;
    cacheStore.setItem(id, "done");
    window.state.controller = null;
    window.el.composer.dispatchEvent(new CustomEvent("sune:newSuneResponse", { detail: { message: window.state.messages.find((m) => m.id === id) } }));
  };
  if (!j || j.rid !== id) {
    if (j && j.error) {
      const t = prevText + "\n\n" + j.error;
  ...
  const text = j.text || "", images = j.images || [], isDone = j.error || j.done || j.phase === "done";
  const content = [];
  if (text) content.push({ type: "text", text });
  content.push(...images);
  const displayText = window.partsToText(content);
  window.renderMarkdown(bubble, displayText, { enhance: false });
  if (isDone) {
    finalise(displayText, content);
    return false;
  }
...
function setupEventListeners() {
  ...
  $(el.fileInput).on("change", async () => {
    ...
    for (const f of files) {
      const at = await window.toAttach(f).catch(() => null);
  ...
  $(el.composer).on("submit", async (e) => {
    ...
    state.busy = true;
    setBtnStop();
    const a = SUNE.active, suneMeta = { sune_name: a.name, model: SUNE.model, avatar: a.avatar || "" }, streamId = window.sid(), suneBubble = addSuneBubbleStreaming(suneMeta, streamId);
    suneBubble.dataset.mid = streamId;
    const assistantMsg = Object.assign({ id: streamId, role: "assistant", content: [{ type: "text", text: "" }] }, suneMeta);
    ...
    const onDelta = (delta, done) => {
      if (done) {
        ...
        else if (delta.text) buf += delta.text;
        const textPart = assistantMsg.content.find((p) => p.type === "text");
        if (textPart) textPart.text = buf;
        else if (buf) assistantMsg.content.unshift({ type: "text", text: buf });
        setBtnSend();
        state.busy = false;
        ...
        if (delta.text) buf += delta.text;
        if (delta.images) assistantMsg.content.push(...delta.images);
        const textPart = assistantMsg.content.find((p) => p.type === "text");
        if (textPart) textPart.text = buf;
  ...
  $(el.importInput).on("change", async () => {
    ...
    try {
      ...
      if (importMode === "sunes") {
        ...
        const incoming = list.map((a) => window.makeSune(a || {}));
        const map = {};
        incoming.forEach((s) => {
          if (!s.id) s.id = gid();
        ...
        const idx = Object.fromEntries(SUNE.list.map((s) => [s.id, s]));
        ...
        SUNE.save();
        if (data.activeId && SUNE.list.some((x) => x.id === data.activeId)) SUNE.setActive(data.activeId);
        renderSidebar();
        await reflectActiveSune();
        state.currentThreadId = null;
        clearChat();
...
Object.assign(window, utils, { SUNE, THREAD, USER, state, el, ...api, ...ui });
async function init() {
